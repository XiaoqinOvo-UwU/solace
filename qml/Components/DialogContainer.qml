...
Dialog {
    ...
    // optional: an item BEHIND the dialog (e.g. the app frame). When set, the
    // dialog gets a real frosted-glass backdrop instead of a flat fill.
    ...
    background: Item {
        // real blur of whatever sits behind the dialog (skipped when no source)
        GlassBackdrop {
            id: backdrop
            anchors.fill: parent
            cornerRadius: Theme.rXl
            backdropSource: root.backdropSource
        }
        Rectangle {
            anchors.fill: parent
            // with a blurred plate behind, a lighter tint keeps white text
            // readable; without a source fall back to the previous solid fills
            color: root.backdropSource !== null
                 ? Qt.rgba(0.075, 0.08, 0.095, 0.76)
                 : (Theme.glassMode ? Qt.rgba(0.14, 0.14, 0.16, 0.94) : Theme.cardFill)
            radius: Theme.rXl
    ...
    // a modal blocks changes behind it, so one grab per open is enough
    onOpened: backdrop.update()
